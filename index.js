...
async function handleInviteLinkDetection(message) {
  ...
  if (matches) {
    console.log(`📋 Found invite links:`, matches);
    ...
    const hasUnallowedInvite = matches.some(invite => {
      ...
      console.log(`🔍 Checking invite code: "${inviteCode}" against whitelist:`, ALLOWED_INVITE_LINKS);
      
      const isAllowed = ALLOWED_INVITE_LINKS.includes(inviteCode);
      console.log(`✅ Is "${inviteCode}" allowed?`, isAllowed);
      
      // Check if this invite code is in the whitelist
      return !isAllowed;
    });
    
    console.log(`🚨 Has unallowed invite?`, hasUnallowedInvite);
